...
- `graduate/field/index.html`: 학교를 먼저 정하지 않는 분야 중심 통합 연구실 추천
...
5. 배포 후 첫 화면의 세 선택지, 인턴 서비스, 학교별 연구실 추천, 분야별 통합 연구실 추천과 학교별 서비스 링크를 확인합니다.
...
- 버튼 클릭, 서비스 선택, 학교 선택, 큰 분야 및 세부 분야 선택, 검색 실행, 더보기, 외부 링크 이동을 이벤트로 추가 집계
...
인턴, 학교별 서비스와 분야별 통합 서비스는 대용량 데이터와 실행 코드를 각각 `data.js`, `app.js`로 분리했습니다. GitHub에 올릴 때 이 파일들을 누락하면 서비스가 작동하지 않습니다.
...
## 뉴바이올로지 추천과목·인턴 업무 근거

DGIST 공식 2025 연구실별 추천 코스트리와 2026 대학원 하계 인턴 참여 연구실 PDF를 교수명 기준으로 연결했습니다. 뉴바이올로지학과 22명 중 추천과목 21명, 인턴 업무 17명의 공식 근거가 카드에 표시됩니다. PDF 미등재 항목은 추정하지 않습니다.

## 2026-07-12 데이터 기반 검색 실패 복구 업데이트

초기 GoatCounter 집계에서 직접 입력 검색 197회 중 38회가 결과 0개였고, 10자 이하 직접 입력은 86회 중 26회가 0개였습니다. 대표 분야 버튼 검색 190회는 모두 결과가 표시돼, 짧은 사용자 표현과 내부 연구 키워드 사이의 연결을 우선 개선했습니다.

- 기존 검색 결과가 있으면 점수와 순서를 그대로 유지
- 직접 입력 결과가 0개일 때만 짧은 검색어를 브라우저 내부에서 보수적으로 보강
- 그래도 결과가 없으면 넓은 분야 선택형 복구 패널 제공
- 검색어 원문과 사용자 ID는 수집하지 않음
- 자동 보강 및 복구 성과를 집계 이벤트로 재측정 가능
- 한국시간 필터, 페이지 경로 정규화, 신규 이벤트 분석을 지원하는 분석기 포함

상세 내용은 `DATA_DRIVEN_QUERY_RECOVERY_REPORT.md`와 `ANALYTICS_GUIDE.md`를 확인하세요.
