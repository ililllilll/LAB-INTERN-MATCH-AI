(function (root, factory) {
  const api = factory(root && root.DGIST_ENERGY_CURATED_DB);
  if (typeof module === "object" && module.exports) module.exports = api;
  if (root) root.DGISTEnergySearchEngine = api;
  ...
  const STOP = new Set(["교수","교수님","연구","연구실","랩","랩실","추천","소개","찾아줘","관련","분야","하는","하시는","하고","싶어","싶습니다","dgist","디지스트","에너지공학과","에너지"]);
  const BROAD_INTENTS = new Set(["battery","solar-cell","catalysis","polymer","soft-electronics","wearable","computational","electrochemistry","sensor"]);
  ...
  function scoreProfile(profile, query) {
    ...
    // 여러 세부 개념을 함께 입력한 경우 모든 구체 개념을 만족해야 합니다.
    // 예: "CO2 전환 촉매", "전고체 계면", "고분자 광촉매".
    ...
    let score = 0;
    score += specificMatched.length * 1900;
    score += broadMatched.length * 500;
    score += hits.strong.length * 520;
    score += hits.related.length * 75;
    score += tokenHits.length * 150;
    if (specific.length && specificMatched.length === specific.length) score += 950;

    [...specificMatched, ...broadMatched].forEach((item) => {
      const order = ((db.intentPriority || {})[item.id] || []);
      const rank = order.indexOf(profile.professor);
      if (rank >= 0) score += Math.max(180, 1300 - rank * 145);
    });

    if (contains(profile.labName, q)) score += 500;
    if (exactStrong) score += 1500;
    if (score < (specific.length ? 950 : 260)) return null;
