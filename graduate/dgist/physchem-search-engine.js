(function (root, factory) {
  const api = factory(root && root.DGIST_PHYSCHEM_CURATED_DB);
  if (typeof module === "object" && module.exports) module.exports = api;
  if (root) root.DGISTPhysChemSearchEngine = api;
  ...
  const STOP = new Set(["교수","교수님","연구","연구실","랩","랩실","추천","소개","찾아줘","관련","분야","하는","하시는","하고","싶어","싶습니다","dgist","디지스트","화학물리학과","화학물리"]);
  const BROAD_INTENTS = new Set(["quantum-general","spectroscopy-general","catalysis-general","sensor-general"]);
  ...
  function contains(text, term) {
    const source = normalize(text), needle = normalize(term);
  ...
  function meaningfulTokens(query) {
    return unique(normalize(query).split(/\s+/).filter((token) => {
      ...
      if (/^[a-z0-9]+$/i.test(token)) return token.length >= 2;
  ...
  function scoreProfile(profile, query) {
    ...
    // 서로 다른 세부 개념이 함께 들어오면 모든 구체 개념을 만족해야 합니다.
    // 예: "초전도 양자컴퓨팅", "DFT 초고속분광".
    if (!exactStrong && specific.length >= 2 && specificMatched.length < specific.length) return null;
    if (!exactStrong && specific.length && !specificMatched.length) return null;
    ...
    if (!specific.length && broad.length && !broadMatched.length && !exactStrong) return null;

    let score = 0;
    score += specificMatched.length * 2100;
    score += broadMatched.length * 400;
    score += hits.strong.length * 560;
    score += hits.related.length * 70;
    score += tokenHits.length * 140;
    if (specific.length && specificMatched.length === specific.length) score += 1000;
    [...specificMatched, ...broadMatched].forEach((item) => {
      const order = ((db.intentPriority || {})[item.id] || []);
      const rank = order.indexOf(profile.professor);
      if (rank >= 0) score += Math.max(180, 1400 - rank * 150);
    });
    if (contains(profile.labName, q)) score += 500;
    if (exactStrong) score += 1600;
    if (score < (specific.length ? 1000 : 260)) return null;

    const evidence = unique([...specificMatched.flatMap(x=>x.hits), ...broadMatched.flatMap(x=>x.hits), ...hits.strong, ...tokenHits, ...hits.related]).slice(0,5);
  ...
  function banner(fieldKey) {
    ...
    return order.map((name,index)=>({profile:byName.get(name),score:100000-index*1000,evidence:[label],coverage:1,intentIds:[fieldKey]})).filter(item=>item.profile);
  }
  function evidenceForProfessor(query, professor) {
    const item = search(query,{limit:40}).find((result)=>result.profile.professor===professor);
